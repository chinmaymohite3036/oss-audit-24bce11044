...
show_usage() { 
    ...
    echo "  $0 /var/log/dnf.log warning" 
...
if [ -z "$LOGFILE" ]; then 
    echo -e "${YELLOW}⚠  No log file specified.${NC}" 
    echo "" 
    find_libreoffice_logs 
    echo "" 
    show_usage 
...
if [ $COUNT -gt 0 ]; then 
    ...
    echo "------------------------" 
    grep -in "$KEYWORD" "$LOGFILE" | tail -5 | while IFS=: read -r LINE_NUM LINE_CONTENT; d
o 
    ...
    # Do-while style retries for empty file? (File is not empty, but we can demonstrate con
cept) 
    echo "📊 STATISTICS" 
    echo "-------------" 
